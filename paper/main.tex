...
total (\texttt{outputs/experiments/phase1\_*.csv}) -- is summarized in
\Cref{fig:baselines-heatmap}, collapsing the representation axis by
taking the best of the three for each (task, model, split) cell so the
figure stays legible. The same values, viewed as a 3D bar chart for the
stratified-random split specifically, are in
\Cref{fig:baselines-3d}: the two low-scoring tasks (\texttt{family},
\texttt{tool}) are visibly a different regime from the other three
across every model, not just weaker on average, which is easier to see
as a height discontinuity than to read off a table of numbers.
...
\begin{figure}[htbp]
  ...
  \caption{The stratified-random slice of \Cref{fig:baselines-heatmap}
    as a 3D bar chart, best representation per (task, model) cell.}
