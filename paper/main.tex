...
output rather than official captions. Aggregate detection-rate
comparisons across \texttt{Transcript\_Provider} do not show an obvious
gap, which is not the same as no effect: individual mistranscriptions
of rare jargon can matter without moving an aggregate rate, since a
single missed \texttt{rclone} mention changes that video's
\texttt{Tool\_Rclone} feature from 1 to 0 regardless of how many other
videos were transcribed correctly.

Source audio is not stored locally (the released dataset ships
transcripts, not audio), so the originally planned manual
audio-vs-transcript listening check is automated here instead: for a
random sample of 40 \texttt{whisper\_base}-transcribed videos (the
smallest, least accurate checkpoint), the opening 6 minutes of audio
was re-fetched and re-transcribed with Whisper's \texttt{small}
checkpoint, treated as a stronger reference, and every occurrence of
six fixed jargon terms (\texttt{LockBit}, \texttt{Mimikatz},
\texttt{PsExec}, \texttt{Cobalt Strike}, \texttt{rclone},
\texttt{BloodHound}, plus known phonetic-mangling variants) in the
reference transcript was checked against the corresponding stored
\texttt{whisper\_base} transcript. 35 of 40 videos were successfully
re-fetched and re-transcribed (5 audio-download failures); across those
35, the six jargon terms appeared at all in only 3 (video, term)
instances within the sampled opening 6 minutes -- jargon density is
low even in ransomware-focused content -- and 1 of those 3 was missed
entirely by \texttt{whisper\_base}: \texttt{rclone}, mentioned once in
the stronger reference transcript for one video and zero times
(correct spelling or a known mangled variant) anywhere in that video's
stored transcript, while \texttt{bloodhound} (same video) and
\texttt{cobalt strike} (18 mentions in another video) were both
correctly detected. A miss rate of 1/3 from a spot-check this small
(3 instances) is not a precise estimate and should not be read as one --
the honest conclusion is qualitative, not quantitative: jargon-term
mistranscription is a real, observed failure mode of this corpus's
dominant transcription method, consistent with Section~\ref{sec:perturbation}'s
simulated-ASR-noise sweep, and a larger, unabridged spot-check (full
video length, not a 6-minute cap; larger sample) is the natural
follow-up this finding motivates rather than resolves.
...
\caption{Best stratified-CV macro-F1 per task, across every representation
  ...
  (\S\ref{sec:gold-scope}). Mean~$\pm$~std is across 5 stratified-random
  split seeds (0--4) at the reported (representation, model) cell, model
  \texttt{random\_state} held fixed -- see the paragraph below the table.}
\label{tab:results}
\begin{tabular}{lllr}
\toprule
Task & Best representation & Best model & Macro-F1 (mean $\pm$ std, 5 seeds) \\
\midrule
Ransomware-relevant & TF-IDF     & GBM    & $0.897 \pm 0.006$ \\
Dominant tactic      & structured & GBM    & $0.806 \pm 0.012$ \\
Platform signal       & TF-IDF     & GBM    & $0.616 \pm 0.008$ \\
Tool mentions         & TF-IDF     & LogReg & $0.166 \pm 0.032$ \\
Family                 & embedding  & LogReg & $0.115 \pm 0.019$ \\
\bottomrule
\end{tabular}
\end{table}

The single-seed point estimates used everywhere else in this paper
(e.g.\ the 0.813 quoted for \texttt{dominant\_tactic} elsewhere) are
each the seed-0 value above, which matches its 5-seed mean closely in
every case ($\leq$0.02 absolute difference) -- the reported numbers
were not outliers relative to the seed distribution. Std is smallest
for \texttt{relevance} (0.006) and largest for \texttt{tool} (0.032,
$\sim$20\% of its own mean), consistent with \texttt{tool}'s
already-noted instability across CV protocols (\Cref{tab:baselines-full}).
Only the stratified-random split's train/test partition varies with the
seed here; \texttt{channel\_grouped} (\texttt{GroupKFold}) and
\texttt{temporal} (a fixed pre/post-2024 cutoff) are both deterministic
single partitions with no random component
(\texttt{Scripts/rk\_pipeline/splits.py}), so a seed sweep is not a
meaningful question for those two and is not computed for them. Model
\texttt{random\_state} (RF/GBM) is likewise held fixed at 0 throughout:
this table isolates variance from which videos land in train vs.\ test
under 5 different stratified partitions specifically, not full
stochastic-training variance, and is reported as exactly that rather
than a broader claim (\texttt{outputs/experiments/phase1\_multiseed.json},
\texttt{Scripts/phase1\_multiseed.py}).
